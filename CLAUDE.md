...
- **`uploadOnSave` aktiviert den FileSystemWatcher automatisch.** Seit dem KI-Agent-Fix (v2.0.0 Patch) gilt: sobald ein Profil `uploadOnSave: true` (oder `watcher.enabled && watcher.autoUpload`) hat, ruft `extension.ts activate()` nach `autoStart()` automatisch `commandHandler.autoStartUploadOnSaveWatchers()` auf. Dieses erstellt pro Workspace-Folder einen `FileWatcher`, registriert `vscode.workspace.createFileSystemWatcher(...)` und startet den Polling-Loop. Hintergrund: Tools wie Cursor, Cline, Continue oder `git apply` schreiben direkt auf Disk ohne VS-Code-Save-Flow — `onDidSaveTextDocument` sieht diese Writes nicht. Manuell-nur-Profile (`uploadOnSave: false && autoUpload: false`) bleiben aussen vor (per-Profil-Filter in `handleFileChange`).
- **`awaitWriteFinish` ist BEWUSST deaktiviert.** `vscode.workspace.createFileSystemWatcher(...)` laeuft OHNE die Option `awaitWriteFinish: { stability: 500 }`. Hintergrund: bei langlaufenden KI-Refactorings (Cursor/Cline schreiben 1-3 s kontinuierlich) wird der VS-Code-Timer bei jedem write-chunk zurueckgesetzt, loest am Ende nie aus, und der finale Datei-State wird nie gemeldet. Der eigene Debounce (`debounceMs = 500`) in `handleFileChange` collapse-t mehrere schnell feuernde `onDidChange`-Events zu einem Upload. Siehe `src/core/fileWatcher.ts` fuer den ausfuehrlichen Kommentar-Block.
